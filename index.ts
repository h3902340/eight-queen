...
let boardWidth: number = 8;
let boardHeight: number = 8;
...
function drawBoard(queenSpot: number[]) {
...
function getNonAttackingPairCount(queenSpot: number[]): number {
    let count = 0;
    for (let j = 0; j < boardWidth; j++) {
        for (let j2 = j + 1; j2 < boardWidth; j2++) {
            if (Math.abs(j2 - j) == Math.abs(queenSpot[j2] - queenSpot[j])) continue;
            if (queenSpot[j2] == queenSpot[j]) continue;
            count++;
        }
    }
    return count;
